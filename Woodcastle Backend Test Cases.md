...
**Tested:** 13 Aug 2026 (original suite). Re-run 25 Aug 2026 for Spec (4) additions: PROD-10–14, MISC-05, MISC-06, REV-01–05, GREV-01, and Cloudinary WebP signed uploads (PROD-06, MISC-01). Admin login `admin` / `admin123`. QA data created then cleaned up. 2FA was enabled for AUTH-03–06 then disabled again (AUTH-07). UI-only cases marked N/A because this repo has no admin panel or public Next.js site.
...
Method: API. PROD-03, PROD-04, PROD-07 need admin/public UI. PROD-06 is the Cloudinary signature endpoint (section 5b), including the fixed `format: "webp"` signed param. PROD-10–14 cover duplicate product name detection (section 5a3), including stripping a previously-suggested code so names never stack (`Dining Chair DC-02 DC-03`).
...
| PROD-06 | Request a Cloudinary signed-upload signature for a product image | `POST /api/admin/upload/signature` with admin JWT and optional `{ folder: "products" }`. Confirm `POST /api/admin/upload` is not a file-upload route | 200 with `{ signature, timestamp, apiKey, cloudName, folder, format: "webp" }`. `format` is part of the signed params so Cloudinary converts the upload to WebP. The file never hits this backend — `POST /api/admin/upload` stays 404 by design. Frontend uploads to Cloudinary with those same fields, then saves the `secure_url` on `POST/PATCH /api/admin/products` | 200. Body includes `signature`, `timestamp`, `apiKey`, `cloudName: "upuirzdo"`, `folder: "products"`, `format: "webp"`. Signature matches Cloudinary sign of `{ timestamp, folder, format }` (not folder-only). `POST /api/admin/upload` → 404 `Route not found`. Drag-and-drop UI is frontend-only (N/A in this repo) | Pass |
...
Method: API. MISC-01 is the Cloudinary signature endpoint with `folder: "offers"` (section 5b), including `format: "webp"` and allowed folders `products, categories, offers, blog, reviews`. MISC-03 public `/blog` pages are frontend; verified via public blog API.

| ID | Test | Steps | Expected Result | Actual Result | Pass/Fail |
|---|---|---|---|---|---|
| MISC-01 | Request a Cloudinary signed-upload signature for an offer banner | `POST /api/admin/upload/signature` with `{ folder: "offers" }`, then save the Cloudinary `secure_url` as `bannerImage` on `POST /api/admin/offers` | 200 with `folder: "offers"`, `format: "webp"`, plus `signature`, `timestamp`, `apiKey`, `cloudName`. Also accepts `folder: "reviews"`. Invalid folders are rejected. No `POST /api/admin/offers/upload` file route — same signed-upload contract as products | 200. `folder: "offers"`, `format: "webp"` with signature fields present. `folder: "reviews"` also 200 with `format: "webp"`. Invalid folder `invoices` → 400 `Invalid folder. Allowed values: products, categories, offers, blog, reviews` | Pass |
